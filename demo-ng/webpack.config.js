...
module.exports = env => {
    ...
    // if (isAnySourceMapEnabled) {
    //     config.plugins.push(new webpack.SourceMapDevToolPlugin({
    //         filename: '[name].js.map',
    //         exclude: ['vendor.js', 'vendor.js.map'],
    //     }));
    // }

    if (uglify && isAnySourceMapEnabled) {
        const sourceMapDir = resolve(join(dist, dirname(sourceMapFilename)));

        config.plugins.push(new SentryCliPlugin({
            include: [resolve(dist), sourceMapDir],
            ignoreFile: '.sentrycliignore',
            ignore: ['node_modules', 'Resources', 'webpack.config.js'],
            urlPrefix: 'file:///app',
            configFile: 'sentry.properties',
            release: 'org.nativescript.demong-1.0',
        }));
    }
